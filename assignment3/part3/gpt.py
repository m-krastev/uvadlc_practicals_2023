...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        y = (
            self.attn_dropout(
                F.softmax(
                    q @ k.transpose(-2, -1) / math.sqrt(v.size(-1))
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            ...
            if top_k is not None:
                # get the minimum (its the last one, but i prefer the clarity)
                remove_indices = logits < torch.topk(logits, top_k).values.min()
                # zero out the logits we don't care about
                logits[remove_indices] = -torch.inf
            
                probs = F.softmax(logits, dim=-1)
                catt = probs.multinomial(1) if do_sample else torch.argmax(probs, dim=-1).unsqueeze(-1)
            else:
                catt = [torch.argmax(logits)]
            idx = torch.cat([idx, catt], dim=1)
